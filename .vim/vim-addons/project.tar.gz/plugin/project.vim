...
function! s:Project(filename) " <<<
    ...
    function! s:ConstructInfo(home, c_d, scriptin, scriptout, flags, filter)
        ...
        "添加编译选项debug，release
        if a:build_type[0] != ''
            let retval=retval.' build_type="'.a:build_type.'"'
        endif
        "End
        return retval
    endfunction ">>>
    ...
    function! s:RefreshEntriesFromDir(recursive)
        ...
        "Add by DY
        let build_type = s:GetBuildType(infoline,s:GetCd(infoline,''))
        if build_type=="release"
            set makeprg=OBJ_SUBDIR
        "End
    ...
    "Add by DY
    function! s:GetBuildType(info,home)
        let bulid_type=substitute(a:info,'.*\<build_type=\([^ {]*\).*', '\1', '')
        if (strlen(build_type) == strlen(a:info))
            let build_type=''
        endif
        return build_type
    endfunction
